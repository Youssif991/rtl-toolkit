...
module tb_seq_detector;
    ...
    always @(negedge clk) begin : check
        if (rstn && (out !== expected_out)) begin
            errors = errors + 1;
            $display("FAIL at time %0t: in=%b | dut_out=%b expected_out=%b (ref_state=%b)", $time,
                     in, out, expected_out, ref_state);
        end
    end
    ...
    initial begin
        $monitor("Time=%0t | rstn=%b in=%b | dut_out=%b expected_out=%b ref_state=%b", $time, rstn,
                 in, out, expected_out, ref_state);
    end
