...
module tb_shift_reg;
    ...
    always @(negedge clk) begin : check
        if (rstn && (out !== expected_out)) begin
            errors = errors + 1;
            $display("FAIL at time %0t: dir=%b en=%b d=%b | dut_out=%b expected_out=%b", $time,
                     dir, en, d, out, expected_out);
        end
    end
    ...
    initial begin : monitor
        $monitor("Time=%0t | rstn=%b en=%b dir=%b d=%b | dut_out=%b expected_out=%b", $time, rstn,
                 en, dir, d, out, expected_out);
    end
